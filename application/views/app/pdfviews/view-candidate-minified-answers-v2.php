...
<?php 
    ...
    if ($total_questions < 50) {
        ...
        $max_rows = ceil(($total_questions - 50) / 50) * 50 + 50;
    }

    $per_page_records = 50;
    $pages = ceil($total_questions / $per_page_records);
    ...
    for($k=1; $k<=$pages; $k++){
        $startIndex = ($k - 1) * $per_page_records;
        $endIndex = $startIndex + $per_page_records - 1;
?>
<div class="container">
    <div class="row">
    ...
    <?php for ($col = 0; $col < $numColumns; $col++): ?>
        <div class="column <?= ($col % 2 == 0) ? 'left-column' : 'right-column' ?>">
            <table cellpadding="0" cellspacing="0" border="1">
                ...
                <tbody>
                    <?php
                        $columnStartIndex = $startIndex + ($col * $maxRowsPerColumn); 
                        $columnEndIndex = $columnStartIndex + $maxRowsPerColumn;
